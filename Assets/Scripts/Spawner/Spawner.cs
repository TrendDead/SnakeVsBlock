...
public class Spawner : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < _repeatCount; i++)
        {
            ...
            GenerateRandomElements(_blockSpawnPoints, _bonusTemplate.gameObject, _bonusSpawnChance, 0.23f);
            MoveSpawner(_distanceBetweenRandomLine);
            GenerateRandomElements(_wallSpawnPoints, _wallTemplate.gameObject, _wallSpawnChance,
                _distanceBetweenFullLine, _distanceBetweenFullLine / 2f);
            GenerateFullLine(_blockSpawnPoints, _blockTemplate.gameObject);
            MoveSpawner(_distanceBetweenRandomLine);
            GenerateRandomElements(_wallSpawnPoints, _wallTemplate.gameObject, _wallSpawnChance,
                _distanceBetweenRandomLine, _distanceBetweenFullLine / 2f);
            GenerateRandomElements(_blockSpawnPoints, _bonusTemplate.gameObject, _bonusSpawnChance, 0.23f);
            MoveSpawner(_distanceBetweenRandomLine);
    ...
    private void GenerateRandomElements(SpawnPoint[] spawnPoints, GameObject generatedElement, int spawnChans, float scaleY = 1, float offsetY = 0)
    {
        for (int i = 0; i < spawnPoints.Length; i++)
        {
            if (Random.Range(0, 100) < spawnChans)
            {
